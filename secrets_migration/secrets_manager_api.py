...
class SecretsManager:
    ...
    def get_secret(self, secret_id: str) -> Union[Dict[str, str], Exception]:
        try:
            return Secret(
                **self.secrets_manager_client.get_secret_value(SecretId=secret_id)
            )
    ...
    def get_secrets(self) -> List[Dict[str, str]]:
        ...
        while True:
            if next_token:
                ...
                ] = self.secrets_manager_client.list_secrets(
                    MaxResults=100, NextToken=next_token
                )
            else:
                incomplete_secrets: Dict[
                    str, Union[List[dict], str]
                ] = self.secrets_manager_client.list_secrets(MaxResults=100)
